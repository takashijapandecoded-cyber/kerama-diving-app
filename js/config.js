...
// ダイビングポイント（優くん優先度順）
//
// 座標はおおよその位置。Marine API は緯度1/12度（約9.3km × 8.3km）の格子に丸める
// （APIが返す中心座標から実測。2026-08-24）。
//
// 「格子が粗いから数百mの精度は不要」とは限らん。黒島北は格子の東の境界まで
// 約330mしかなく、基準地点 LOCATIONS.kerama と同じ格子に落ちとる。そのため
// 黒島北の波・うねりは慶良間沖と常に完全に同じ値が出る＝独立した情報になっとらん。
// さらに LOCATIONS.kerama 自体が島の上に乗っとる（海岸線まで19m・OSMで実測）。
//
// TODO: 優くんに確認してもらい、返事が来たらここを直す。
//       確認用の地図と生成スクリプト: council/2026-08-24_座標確認/
...
//
// ⚠️ コードは推測で書かず、必ず気象庁の実XMLで名前を突き合わせること。
// 2026-08-23、粟国=4734800（実際は与那原町）・渡名喜=4735000（実際は南風原町）と
// 誤っとるのを発見。南風原町は内陸で波浪注意報が構造上出んため、渡名喜ポイントの
// 波関連スコア上限がずっと効いとらんかった（8/22の実XMLで実害を確認）。
// tests/safety.test.mjs が気象庁の実データに対してこの表を毎回検証する。
export const WARNING_AREAS = {
  ...
  aguni:  { codes: ['4735500'], label: '粟国' },              // 粟国村
  tonaki: { codes: ['4735600'], label: '渡名喜' },            // 渡名喜村
};
...
//
// 雷・大雨・濃霧などは意図的に対象外（2026-07-21 たかしさん判断・入れ忘れではない）:
// これらは警報チップとして画面・メールに必ず表示され優くんの目に入るため、
// 点数まで下げるのは過剰という結論。海況（波・風）に直接効く警報だけを上限の対象にする
